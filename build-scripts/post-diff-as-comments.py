...
def hunk_to_suggestions(hunks: list[dict[str, any]]):
    # We want to trim away excess context. Ideally the diff is generated with -U1,
    # but just in case, we split large context pieces into smaller ones. "Large" meaning
    # anything > 3 lines is split into two single line contexts, for the first and last
    # lines in the hunk. That also inserts a hunk boundary.
    ...
    # Now coalesce hunks into suggestions. Each hunk in trimmed hunks should have:
    ...
    # If there is internal context or deletions, we drop the leading and trailing context
    # otherwise we use the leading context as the suggestions anchor, or the trailing context
    # We assume we aren't dealing with an empty file that only has an addition in it.
