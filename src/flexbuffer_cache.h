...
struct parsed_flexbuffer {
    ...
    protected:
        parsed_flexbuffer( std::shared_ptr<flexbuffer_storage> storage );
...
class flexbuffer_cache
{
    ...
    public:
        ...
        shared_flexbuffer parse( fs::path json_file_path, size_t offset = 0 ) noexcept( false );
        shared_flexbuffer parse_and_cache( fs::path json_file_path,
                                           size_t offset = 0 ) noexcept( false ) ;

        static shared_flexbuffer parse_buffer( std::string buffer ) noexcept( false );

    private:
        flexbuffer_cache() = default;
        flexbuffer_cache( flexbuffer_cache && ) = default;

        struct ParsedBuffer;

        // Map of original json file path to cached parsed FlexBuffer
        std::unordered_map<std::string, shared_flexbuffer> cached_files_;
        // Map of input string to cached parsed FlexBuffer
        std::unordered_map<std::string, shared_flexbuffer> cached_buffers_;
