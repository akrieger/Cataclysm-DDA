...
shared_ptr_fast<mm_submap> map_memory::load_submap( const tripoint_abs_sm &sm_pos )
{
    ...
    std::string mm_filename = find_region_filename( p.reg );
    ...
    try {
        ...
        if( world_generator->active_world->has_compression_enabled() ) {
            std::shared_ptr<zzip> z = zzip::load( ( mm_dir / mm_filename + ".zzip" ).get_unrelative_path(),
                                                  ( PATH_INFO::world_base_save_path() / "mmr.dict" ).get_unrelative_path() );
            if( !z ) {
                return nullptr;
            }
            if( !read_from_zzip_optional( z, mm_filename, [&]( std::istream & is ) {
...
bool map_memory::save( const tripoint_abs_ms &pos )
{
    ...
    for( auto &it : regions ) {
        ...
        if( !reg.is_empty() ) {
            const std::string mm_filename = find_region_filename( regp );
            ...
            if( world_generator->active_world->has_compression_enabled() ) {
                std::shared_ptr<zzip> z = zzip::load( ( path + ".zzip" ).get_unrelative_path(),
                                                      ( PATH_INFO::world_base_save_path() / "mmr.dict" ).get_unrelative_path() );
                if( z ) {
