...
bool WORLD::set_compression_enabled( bool enabled ) const
{
    ...
    static_popup popup;
    if( enabled ) {
        std::filesystem::path maps_dict =
            PATH_INFO::maps_compression_dictionary_path().get_unrelative_path();
        ...
        std::filesystem::path maps_dict = ( folder_path() / "maps.dict" ).get_unrelative_path();
