...
constexpr unsigned int kFooterChecksumMagic = 2;
...
bool zzip::delete_file( const std::filesystem::path &zzip_relative_path )
{
    // This is inefficient as sin but this should be called so rarely
    // that I don't care to make this faster.
    // Actually, sin is usually very efficient.

    zzip_footer footer{ footer_ };
    if( !has_file( zzip_relative_path ) ) {
        return false;
    }
    ...
    std::string relative_path_string = zzip_relative_path.generic_u8string();
    compressed_entries.erase( std::remove_if( compressed_entries.begin(),
    compressed_entries.end(), [&]( const compressed_entry & entry ) {
        bool found = entry.path == relative_path_string;
        did_erase = did_erase || found;
        return found;
    } ), compressed_entries.end() );

    // We did check earlier if we had the file, but who knows.
    if( !did_erase ) {
        return false;
    }

    // Rewrite the footer from scratch, with all entries but the removed one.
    if( !update_footer( JsonObject{}, 0, compressed_entries ) ) {
        return false;
    }
    // Force a compaction to rewrite the file without the removed entry.
    return compact( 0 );
}
