    ...
    const char *source_filename_opt ) noexcept( false )
{
    ...
    if( !parser.ParseFlexBuffer( buffer, source_filename_opt, &fbb ) ) {
        ...
        if( strncmp( parser.error_.c_str(), "EOF", 3 ) != 0 ) {
            ...
            if( sscanf( parser.error_.c_str(), "%llu:%llu: %n", &line, &col, &error_offset ) != 2 ) {
                line = 0;
                col = 0;
            }
        }
        ...
        jsin.error( parser.error_.substr( error_offset ), col - 1 );
    }
...
struct flexbuffer_vector_storage : flexbuffer_storage {
    ...
    flexbuffer_vector_storage( std::vector<uint8_t> &&buffer ) : buffer_{ std::move( buffer ) } {}
...
struct flexbuffer_mmap_storage : flexbuffer_storage {
    ...
    flexbuffer_mmap_storage( std::shared_ptr<mmap_file> mmap_handle ) : mmap_handle_{ std::move( mmap_handle ) } {}
...
struct file_flexbuffer : parsed_flexbuffer {
        ...
        virtual ~file_flexbuffer() override = default;
        ...
        virtual std::unique_ptr<std::istream> get_source_stream() const override {
...
struct string_flexbuffer : parsed_flexbuffer {
        ...
        virtual ~string_flexbuffer() override = default;
...
{
    public:
        static std::unique_ptr<flexbuffer_disk_cache> init_from_folder( fs::path cache_path ) {
        ...
        bool has_cached_flexbuffer_for_json( fs::path json_source_path ) {
            return cached_flexbuffers_.count( json_source_path.u8string() ) > 0;
        }

        fs::file_time_type cached_mtime_for_json( fs::path json_source_path ) {
        ...
        std::shared_ptr<flexbuffer_mmap_storage> load_flexbuffer_if_not_stale( fs::path json_source_path ) {
            ...
            auto mmap_handle = mmap_file::map_file( disk_entry->second.flexbuffer_path.u8string() );
            if( !mmap_handle ) {
                return storage;
            }
    ...
    private:
        flexbuffer_disk_cache( fs::path cache_path ) : cache_path_{ std::move( cache_path ) } {}
...
flexbuffer_cache::~flexbuffer_cache() {}

flexbuffer_cache &flexbuffer_cache::global_cache()
{
    static flexbuffer_cache global_cache_ = ( []() -> flexbuffer_cache {
        flexbuffer_cache cache;
        cache.disk_cache_ = flexbuffer_disk_cache::init_from_folder( fs::u8path( PATH_INFO::cache_dir() ) / "flexbuffers" );
        return cache;
    } )();
...
size_t flexbuffer_cache::drop_cache() noexcept
{
...
std::shared_ptr<parsed_flexbuffer> flexbuffer_cache::parse_buffer( std::string json )
{
    auto fb = parse_json_to_flexbuffer_( json.c_str(), nullptr );
    auto storage = std::make_shared<flexbuffer_vector_storage>( std::move( fb ) );
    return std::make_shared<string_flexbuffer>( std::move( storage ), std::move( json ) );
}
