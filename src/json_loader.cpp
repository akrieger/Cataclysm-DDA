        ...
        size_t offset )
{
    ...
    std::shared_ptr<parsed_flexbuffer> buffer = cache.parse_and_cache(
...
cata::optional<JsonValue> json_loader::from_path_at_offset_opt( fs::path source_file,
        size_t offset ) noexcept( false )
{
    return from_path_at_offset_opt( cata_path{ cata_path::root_path::unknown, source_file }, offset );
}

cata::optional<JsonValue> json_loader::from_path_opt( fs::path source_file ) noexcept( false )
{
    return from_path_opt( cata_path{ cata_path::root_path::unknown, source_file } );
}
