...
flexbuffer_cache &save_cache_for_save( const std::string &save_name )
{
    static std::unordered_map<std::string, std::shared_ptr<flexbuffer_cache>> caches;
    auto it = caches.find( save_name );
    if( it != caches.end() ) {
        return *it->second;
    }
    fs::path cache_path = fs::u8path( PATH_INFO::savedir() + save_name + "/cache" );
    std::shared_ptr<flexbuffer_cache> cache = std::make_shared<flexbuffer_cache>( std::move(
                cache_path ), fs::u8path( PATH_INFO::savedir() + save_name ) );
    return *caches.emplace( save_name, cache ).first->second;
}

flexbuffer_cache &mod_cache( std::string mod_name )
{
    static std::unordered_map<std::string, std::shared_ptr<flexbuffer_cache>> caches;
    auto it = caches.find( mod_name );
    if( it != caches.end() ) {
        return *it->second;
    }
    fs::path cache_path = PATH_INFO::moddir() + mod_name + "/cache";
    std::shared_ptr<flexbuffer_cache> cache = std::make_shared<flexbuffer_cache>( std::move(
                cache_path ), PATH_INFO::moddir() + mod_name );
    return *caches.emplace( mod_name, cache ).first->second;
}

// Will throw if the file doesn't exist.
flexbuffer_cache *get_appropriate_cache( fs::path const &source_file )
{
    fs::path canonical_path = fs::weakly_canonical( source_file );
