...
[[noreturn]] inline void TextJsonValue::string_error( const std::string &err, int offset ) const
{
    seek().string_error( err, offset );
}

[[noreturn]] inline void TextJsonValue::throw_error( const std::string &err, int offset ) const
{
    seek().error( err, offset );
}

[[noreturn]] inline void TextJsonValue::throw_error_after( const std::string &err ) const
{
    seek().skip_value();
    jsin_.error( err );
}
