#pragma once
#ifndef CATA_SRC_TEXT_JSON_H
#define CATA_SRC_TEXT_JSON_H
...
/* Cataclysm-DDA homegrown TextJson tools
 * copyright CC-BY-SA-3.0 2013 CleverRaven
 *
 * Consists of six TextJson manipulation tools:
 * TextJsonIn - for low-level parsing of an input TextJson stream
 * TextJsonOut - for outputting TextJson
 * TextJsonObject - convenience-wrapper for reading TextJson objects from a TextJsonIn
 * TextJsonArray - convenience-wrapper for reading TextJson arrays from a TextJsonIn
 * TextJsonSerializer - inheritable interface for custom datatype serialization
 * TextJsonDeserializer - inheritable interface for custom datatype deserialization
 *
...
class TextJsonError : public std::runtime_error
{
    public:
        explicit TextJsonError( const std::string &msg );
 ...
 * with methods for reading TextJson data directly from the stream.
 *
 ...
 *     jsin.start_array(); // throws TextJsonError if array not found
 ...
 *
 * If the TextJson structure is not as expected,
...
class TextJsonIn
{
    ...
    public:
        explicit TextJsonIn( std::istream &s ) : stream( &s ) {}
        ...
        // if throw_on_error then throws TextJsonError rather than returning false.
        ...
        bool read( std::pair<T, U> &p, bool throw_on_error = false ) {
            if( !test_array() ) {
                return error_or_false( throw_on_error, "Expected TextJson array encoding pair" );
            }
            try {
                start_array();
        ...
        auto read( T &v, bool throw_on_error = false ) -> decltype( v.front(), true ) {
            if( !test_array() ) {
                return error_or_false( throw_on_error, "Expected TextJson array" );
            }
            try {
                start_array();
                v.clear();
        ...
        bool read( std::array<T, N> &v, bool throw_on_error = false ) {
            ...
            try {
                ...
                for( size_t i = 0; i < N; ++i ) {
                    if( end_array() ) {
                        if( throw_on_error ) {
                            error( "TextJson array is too short" );
                        }
                        return false; // TextJson array is too small
                    }
        ...
        bool read( T &m, bool throw_on_error = true ) {
            if( !test_object() ) {
                return error_or_false( throw_on_error, "Expected TextJson object" );
            }
            try {
                start_object();
                m.clear();
 ...
 * =======
 *
 * The TextJsonOut class provides a straightforward interface for outputting TextJson.
 *
 * It wraps a std::ostream, providing methods for writing TextJson data directly.
 *
 * Typical usage might be as follows:
 *
 *     TextJsonOut jsout(myostream);
 *     jsout.start_object();          // {
 ...
 * as well as anything inheriting the TextJsonSerializer interface,
...
class TextJsonOut
{
    ...
    public:
        explicit TextJsonOut( std::ostream &stream, bool pretty_print = false, int depth = 0 );
        TextJsonOut( const TextJsonOut & ) = delete;
        TextJsonOut &operator=( const TextJsonOut & ) = delete;
        ...
        // write data to the output stream as TextJson
        void write_null();
        ...
        /// Overload that calls a global function `serialize(const T&,TextJsonOut&)`, if available.
        template<typename T>
        auto write( const T &v ) -> decltype( serialize( v, *this ), void() ) {
            serialize( v, *this );
        }

        /// Overload that calls a member function `T::serialize(TextJsonOut&) const`, if available.
        template<typename T>
        ...
        void write( const TextJsonSerializer &thing );
        // This is for the string_id type
        template <typename T>
 ...
 * ==========
 *
 * The TextJsonObject class provides easy access to incoming TextJson object data.
 *
 ...
 * TextJsonObject will throw a TextJsonError if they are not found,
 ...
 * in lang/extract_TextJson_strings.py to be translatable.
 *
 * The "points" member here is not required.
 * If it is not found in the incoming TextJson object,
 ...
 * --------------------------------------------
 *
 * TextJsonObjects can test for member type with has_int(name) etc.,
 * and for member existence with has_member(name).
 *
 * They can also read directly into compatible data structures,
 * including sets, vectors, maps, and any class inheriting TextJsonDeserializer,
...
class TextJsonObject
{
    ...
    public:
        ...
        std::string str() const; // copy object TextJson as string
 ...
 * =========
 *
 * The TextJsonArray class provides easy access to incoming TextJson array data.
 *
 ...
 * TextJsonArray will throw a TextJsonError indicating the problem,
 ...
 * such as maps, sets, vectors, and classes implementing TextJsonDeserializer,
 * using the read_next() and read() methods.
 *
 *     TextJsonArray ja = jo.get_array("custom_datatype_array");
 *     while (ja.has_more()) {
 *         MyDataType mydata; // MyDataType implementing TextJsonDeserializer
 *         ja.read_next(mydata);
 *         process(mydata);
 *     }
 */
...
<code>
for( const TextJsonMember &member : some_TextJson_object )
...
class TextJsonMember : public TextJsonValue
{
    ...
    public:
         ...
         * when this class is used within a generic TextJson tool.
         */
 ...
 * ==============
 *
 * TextJsonSerializer is an inheritable interface,
 ...
 * is define a `void serialize(TextJsonOut&) const` method,
 * which should use the provided TextJsonOut to write its data as TextJson.
 *
 *     class point : public TextJsonSerializer {
 *         int x, y;
 *         void serialize(TextJsonOut &jsout) const {
 *             jsout.start_array();
 *             jsout.write(x);
 *             jsout.write(y);
 *             jsout.end_array();
...
{
    public:
        virtual ~TextJsonSerializer() = default;
        virtual void serialize( TextJsonOut &jsout ) const = 0;
        TextJsonSerializer() = default;
        TextJsonSerializer( TextJsonSerializer && ) = default;
        TextJsonSerializer( const TextJsonSerializer & ) = default;
        TextJsonSerializer &operator=( TextJsonSerializer && ) = default;
        TextJsonSerializer &operator=( const TextJsonSerializer & ) = default;
};

/* TextJsonDeserializer
 * ==============
 *
 * TextJsonDeserializer is an inheritable interface,
 * allowing classes to define how they are to be deserialized,
 * and then treated as a basic type for deserialization purposes.
 *
 * All a class must to do satisfy this interface,
 * is define a `void deserialize(TextJsonIn&)` method,
 * which should read its data from the provided TextJsonIn,
...
class TextJsonDeserializer
{
    public:
        ...
        TextJsonDeserializer &operator=( TextJsonDeserializer && ) = default;
        TextJsonDeserializer &operator=( const TextJsonDeserializer & ) = default;
};

std::ostream &operator<<( std::ostream &stream, const TextJsonError &err );

template<typename T>
void serialize( const cata::optional<T> &obj, TextJsonOut &jsout )
{
    if( obj ) {
        jsout.write( *obj );
    } else {
