...
template<typename T>
T read_from_json_string( const JsonObject &jo, const std::string member_name,
                         const std::vector<std::pair<std::string, T>> &units )
{
    const auto error = [&]( const char *const msg, size_t offset ) {
        jo.throw_error( msg, member_name, offset );
    };

    const std::string s = jo.get_string( member_name );
