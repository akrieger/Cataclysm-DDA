                                 ...
                                 const bool throw_exception ) const
{
    if( !jsin ) {
        if( throw_exception ) {
            throw TextJsonError( std::string( "member lookup on empty object: " ) + name );
        }
...
void TextJsonObject::throw_error( const std::string &err, const std::string &name ) const
{
    mark_visited( name );
    if( !jsin ) {
        throw TextJsonError( err );
    }
...
void TextJsonArray::throw_error( const std::string &err )
{
    if( !jsin ) {
        throw TextJsonError( err );
    }
...
void TextJsonArray::throw_error( const std::string &err, int idx )
{
    if( !jsin ) {
        throw TextJsonError( err );
    }
...
{
    if( !jsin ) {
        throw TextJsonError( "TextJsonObject::get_source_location called when stream is null" );
    }
...
{
    if( !jsin ) {
        throw TextJsonError( "tried to access empty array." );
...
bool TextJsonIn::read( TextJsonDeserializer &j, bool throw_on_error )
{
...
void TextJsonIn::error( const std::string &message, int offset )
{
    std::ostringstream err_header;
    switch( error_log_format ) {
        case error_log_format_t::human_readable:
            err_header << "TextJson error: " << line_number( offset ) << ": ";
            break;
        case error_log_format_t::github_action:
    ...
    if( !stream->good() ) {
        throw TextJsonError( err_header.str() + escape_data( message ) );
    }
    ...
    throw TextJsonError( err_header.str() + escape_data( msg ) );
}

void TextJsonIn::string_error( const std::string &message, const int offset )
{
    if( test_string() ) {
        // skip quote mark
        stream->ignore();
...
bool TextJsonIn::error_or_false( bool throw_, const std::string &message, int offset )
{
    if( throw_ ) {
...
TextJsonOut::TextJsonOut( std::ostream &s, bool pretty, int depth ) :
...
int TextJsonOut::tell()
{
    return stream->tellp();
}

void TextJsonOut::seek( int pos )
{
    stream->clear();
    stream->seekp( pos );
...
void TextJsonOut::write_indent()
{
    std::fill_n( std::ostream_iterator<char>( *stream ), indent_level * 2, ' ' );
}

void TextJsonOut::write_separator()
{
    if( !need_separator ) {
        return;
    }
...
void TextJsonOut::write_member_separator()
{
    if( pretty_print ) {
        stream->write( ": ", 2 );
    } else {
...
void TextJsonOut::start_object( bool wrap )
{
    if( need_separator ) {
        write_separator();
    }
...
void TextJsonOut::start_array( bool wrap )
{
    if( need_separator ) {
        write_separator();
    }
...
void TextJsonOut::write( const std::string &val )
{
    if( need_separator ) {
        write_separator();
    }
...
template<size_t N>
void TextJsonOut::write( const std::bitset<N> &b )
{
    if( need_separator ) {
        write_separator();
    }
...
void TextJsonOut::write( const TextJsonSerializer &thing )
{
    if( need_separator ) {
        write_separator();
    }
...
void TextJsonOut::member( const std::string &name )
{
    write( name );
    write_member_separator();
}

void TextJsonOut::null_member( const std::string &name )
{
    member( name );
    write_null();
}

TextJsonError::TextJsonError( const std::string &msg )
...
template void TextJsonOut::write<12>( const std::bitset<12> & );
