...
std::pair<std::filesystem::path, std::filesystem::path> get_world_root_and_relative_content_path(
...
std::pair<const char *, std::unique_ptr<std::exception>> for_each_zzip_entry(
            std::filesystem::path zzip_path, const std::vector<char> &zzip_contents,
            std::function<std::unique_ptr<std::exception>( const std::string &, uint64_t, const char *, size_t )>
            fn )
{
...
#define RETURN_ERROR(e) return {end_of_last_entry, std::unique_ptr<std::exception>(new e)}
    ...
    while( remaining_len > 0 && ZSTD_isFrame( base, remaining_len ) ) {
        ...
        std::unique_ptr<std::exception> err = fn( filename, checksum, base, frame_size );
        if( err ) {
...
int decompress_to( std::filesystem::path const &zzip_path, std::filesystem::path output_folder,
...
#define RETURN_ERROR(e) return std::unique_ptr<std::exception>(new e)
...
int compress_to( std::filesystem::path const &zzip_path, std::filesystem::path archive_filename,
                 std::filesystem::path input_file, std::filesystem::path const &dict )
{
    ...
    auto [after_last_entry_ptr, err] = for_each_zzip_entry( zzip_path,
                                       zzip_contents, [&]( const std::string & entry_filename,
                                               uint64_t checksum, const char *entry_base,
    size_t zzip_remaining ) -> std::unique_ptr<std::exception> {
    ...
    std::ofstream zip{ zzip_path, std::ios::binary | ( zzip_exists ? std::ios::in : 0 )};
    if( !zip ) {
...
int handle_file_contextual( std::filesystem::path const &input_file )
{
    ...
    std::filesystem::path dict = [&]() -> std::filesystem::path {
        if( parts[0] == kMapsFolder )
        {
            return assert_exists( world_root / kMapsDict );
        }
        if( parts[0] == kOvermapsFolder || parts[0].generic_u8string().find( "o." ) == 0 )
        {
            return assert_exists( world_root / kOvermapsDict );
        }
        if( parts[0].extension() == kMapMemoryExt )
        {
            return assert_exists( world_root / kMapMemoryDict );
        }
        return {};
    }();
